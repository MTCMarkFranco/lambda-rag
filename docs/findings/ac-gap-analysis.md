# Air Canada contract: gap-analysis investigation

> Closes [#6 — P0.1 Investigate AC contract gap=2 (PAY-001, DPA-001)](../../issues/6).
>
...
| Artifact | Path |
|---|---|
| Source contract (parsed) | `out/ac-test/parsed.json` |
| Topic-map projection | `out/ac-test/projected.json` |
| End-to-end review report | `out/ac-test/report.json` |
| Air Canada ruleset | `out/ac-full/ac-policies-ruleset.json` |
...
passed            9
failed            7
gap               2   ← AC-2011-000PAY-001  AC-2011-003DPA-001
score             0.5000
```

---

## Rule 1 — `AC-2011-000PAY-001`  *Payment terms must be 30 days or fewer*
...
**Real gap in the source contract** with respect to the Air Canada policy.
...
language. From the perspective of an AC policy that requires a named
...
`out/ac-full/report.run1.json` vs `report.run2.json` (modulo the AC-test
vs AC-full input differential — which is a *different* contract / ruleset
